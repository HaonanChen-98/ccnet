    ...
    def _training(self):
        ...
        for i in range(self.epochs):
            ...
            for batch_x in self.train_loader:
                # t5 = time.time()
                # if t5 - t6 > 0:  # 检查时间差，计算两个批次之间的训练间隙时间
                    # print('training_gap_time', time.time() - t6)
                # t2 = time.time()
                loss = self.training_forward(batch_x, self.net, self.criterion)
                # print('losstime', time.time() - t2)
                self.net.zero_grad()
                t4 = time.time()
                loss.backward()
                # print('backtime', time.time() - t4)
                t3 = time.time()
                optimizer.step()
                # print('optimizer', time.time() - t3)
